...
class MfaResumeTest(unittest.TestCase):
    ...
    def test_submit_transient_exception_is_not_invalid_code(self) -> None:
        # A Salesforce remoting exception / 5xx must surface as a transient auth error,
        # NOT "invalid code" (so the user isn't told to re-enter a good OTP).
        auth = self._auth([FakeResp(text='[{"statusCode":500,"type":"exception","message":"boom"}]')])
        with self.assertRaises(NativeAuthError) as cm:
            asyncio.run(auth.submit_mfa_code(self._context(), "12345"))
    ...
    def test_resend_failure_raises(self) -> None:
        auth = self._auth([_verify_remoting(False)])
        with self.assertRaises(NativeAuthError):
            asyncio.run(auth.resend_mfa_code(self._context()))

    def test_submit_token_retrieval_failure_raises(self) -> None:
        auth = self._auth([
            _verify_remoting(True),
...
class MfaErrorCodesTest(unittest.TestCase):
    def test_codes_registered(self) -> None:
        ...
        for code in (error_codes.MFA_REQUIRED, error_codes.MFA_CODE_INVALID):
    ...
    def test_mfa_requires_reauth(self) -> None:
